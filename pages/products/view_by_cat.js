...
import { faPencilAlt, faPlus, faTag, faTrash } from "@fortawesome/free-solid-svg-icons";
...
export default function ViewByCat({ me, cat, categoryOptions, cartItems, onAddCartItem, onRemoveCartItem }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-2">
      <Head>
        <title>Products for '{cat.title}'</title>
      </Head>
      ...
      <main className="flex flex-col items-center justify-center w-full flex-1 px-10 pt-4 pb-16 mb-16 bg-base-200 text-center">
        <h1 className="text-3xl sm:text-6xl font-bold bg-primary text-secondary rounded-lg w-full">
          Products for
          <FontAwesomeIcon className="ml-4 mr-2" icon={faTag} size="xs" />
          &rsquo;{cat.title}&rsquo;
        </h1>
        ...
        {me && me.isSuperAdmin &&
          ...
          <div className="py-10 mx-auto w-full lg:grid lg:grid-cols-2 lg:gap-6 lg:justify-center">
            ...
            {cat.products.map(function(product, idx){
              return <div key={idx}>
...
ViewByCat.getInitialProps = ({ query: { me, cat, categoryOptions } }) => {
  return { me, cat, categoryOptions };
}
