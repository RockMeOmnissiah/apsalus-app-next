...
export default function Overview({ me, orders }) {
    ...
    <div className="flex flex-col items-center justify-center py-2">
      ...
      <main className="flex flex-col items-center justify-center w-full flex-1 px-10 pt-4 pb-16 mb-16 bg-base-200 text-center">
        ...
        {me && me.isSuperAdmin &&
          ...
          <div className="pt-2">
            <DataTable
              hiddenColumns={['external_order_id', 'nameData', 'status', 'shipTo', 'totalPrice']}
              columns={[
                {
                  Header: 'NameData',
                  accessor: 'nameData',
              ...
              data={orders.map(function(order, idx){
                return {
                  key: {idx},
                  nameData: order.fullName + ' ' + order.emailAddress,
                  nameDisplay: <div className="w-36">
                    <div className="w-full text-xs sm:text-sm">{order.user.id}</div>
                    <div className="w-full text-xs sm:text-sm">{order.fullName}</div>
                    <div className="text-xs sm:text-sm">{order.emailAddress}</div>
                  </div>,
                  external_order_id: order.external_order_id + ' ' + order.id,
                  external_order_id_display: <div className="w-36">
                    <div className="w-full text-xs sm:text-sm">Ext: {order.external_order_id}</div>
                    <div className="text-xs sm:text-sm">Int: {order.id}</div>
                    <label onClick={()=>{
...
Overview.getInitialProps = ({ query: { me, orders } }) => {
  return { me, orders };
}
